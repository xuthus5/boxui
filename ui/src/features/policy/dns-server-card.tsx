...
export function DNSServerCard({ item, onEdit, onCopy, onDelete }: DNSServerCardProps) {
  ...
  return <><Card size="sm"><CardHeader><CardTitle>{tag}</CardTitle>
    <CardDescription>{inferDNSServerType(item)}</CardDescription><CardAction>
      <Button variant="outline" size="xs" aria-label={`编辑 DNS 服务器 ${tag}`} onClick={onEdit}>
    ...
    <CardFooter className="justify-between gap-2"><div className="hidden gap-1 sm:flex">
      <Button variant="outline" size="icon-xs" aria-label={`复制 DNS 服务器 ${tag}`} onClick={onCopy}><CopyIcon data-icon="inline-start" /></Button>
      <Button variant="destructive" size="icon-xs" aria-label={`删除 DNS 服务器 ${tag}`} onClick={() => setDeleting(true)}><Trash2Icon data-icon="inline-start" /></Button>
    </div><div className="sm:hidden"><DropdownMenu>
      <DropdownMenuTrigger render={<Button variant="outline" size="icon-xs" aria-label={`更多 DNS 服务器 ${tag}`} />}><EllipsisIcon data-icon="inline-start" /></DropdownMenuTrigger>
    ...
    <AlertDialogHeader><AlertDialogTitle>删除 DNS 服务器 {tag}？</AlertDialogTitle>
