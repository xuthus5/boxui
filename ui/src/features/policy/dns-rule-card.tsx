...
function DesktopActions(props: Omit<DNSRuleCardProps, "item" | "onEdit">) {
  ...
  return <div className="hidden gap-1 sm:flex">
    <Button variant="outline" size="icon-xs" aria-label={`复制 DNS 规则 ${number}`} onClick={onCopy}><CopyIcon data-icon="inline-start" /></Button>
    <Button variant="outline" size="icon-xs" aria-label={`上移 DNS 规则 ${number}`} disabled={first} onClick={onMoveUp}><ArrowUpIcon data-icon="inline-start" /></Button>
    <Button variant="outline" size="icon-xs" aria-label={`下移 DNS 规则 ${number}`} disabled={last} onClick={onMoveDown}><ArrowDownIcon data-icon="inline-start" /></Button>
    <Button variant="destructive" size="icon-xs" aria-label={`删除 DNS 规则 ${number}`} onClick={onDelete}><Trash2Icon data-icon="inline-start" /></Button>
  </div>
}
...
function MobileActions(props: Omit<DNSRuleCardProps, "item" | "onEdit">) {
  ...
  return <div className="sm:hidden"><DropdownMenu>
    <DropdownMenuTrigger render={<Button variant="outline" size="icon-xs" aria-label={`更多 DNS 规则 ${index + 1}`} />}><EllipsisIcon data-icon="inline-start" /></DropdownMenuTrigger>
...
export function DNSRuleCard(props: DNSRuleCardProps) {
  ...
  return <><Card size="sm"><CardHeader><CardTitle>DNS 规则 #{number}</CardTitle>
    <CardDescription>{String(item.type ?? "default")}</CardDescription><CardAction>
      <Button variant="outline" size="xs" aria-label={`编辑 DNS 规则 ${number}`} onClick={onEdit}>
    ...
    <AlertDialogHeader><AlertDialogTitle>删除 DNS 规则 #{number}？</AlertDialogTitle>
      <AlertDialogDescription>此操作无法撤销。</AlertDialogDescription></AlertDialogHeader>
    <AlertDialogFooter><AlertDialogCancel>取消</AlertDialogCancel>
      <AlertDialogAction variant="destructive" onClick={confirmDelete}>确认删除</AlertDialogAction></AlertDialogFooter>
