...
- 以下操作仍必须确认：删除文件、大规模重构、shared contract / schema / shared types、根配置 / CI / 依赖 / 环境模板、数据库 / 持久化变更、基础设施或越界改动；“自动本地部署规则”明确授权的 boxd 构建与 `boxd.service` 重启除外。
...
- 任务实际修改仓库文件并成功创建本地 Git 提交后，必须在最终回复前部署该提交对应的 boxd 二进制。
- 使用仓库正式构建流程重新构建前端与 Go 二进制，安全替换 `/usr/local/bin/boxd`，然后重启 `boxd.service`。
- `/usr/local/bin/boxd` 必须保持 `root:boxd` 属主与 `0750` 权限，确保服务用户可以执行且不能写入；不得安装为服务用户无法执行的 `0700 root:root`。
